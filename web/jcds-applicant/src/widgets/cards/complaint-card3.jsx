...
export function ComplaintCard3({
  complaint_id,
  case_type,
  judge_name,
  case_file_number,
  ...
  const handleViewDetails = () => {
    ...
    setTimeout(() => {
      try {
        console.log("Navigating to:", `/home/complaint-details-v3/${complaint_id}`);
        console.log("With state:", { fullComplaintId: complaint_id });
        navigate(`/home/complaint-details-v3/${complaint_id}`, {
  ...
  const shortId = complaint_id ? complaint_id.toString().slice(0, 8).toUpperCase() : "N/A";

  return (
    <div className="bg-white rounded-xl flex flex-col gap-3 w-full max-w-3xl min-h-72 py-4 px-6 shadow-md">
      {/* First Section: Complaint ID and Case Type */}
      <div className="flex flex-col gap-1.5 rounded-md w-full py-2 px-4 min-h-20 border border-gray-200 bg-gray-50">
        ...
        {/* <div className="flex items-center justify-between font-['Montserrat'] font-semibold text-lg leading-none text-[#073954]">
          <span>Case Type</span>
          <span className="text-right">Complaint Case</span>
        </div> */}
      </div>
      ...
      <div className="flex justify-end w-full mt-auto">
        <button
          ...
          className="flex items-center justify-center bg-[#215167] text-white rounded-[10px] font-['Montserrat'] font-semibold text-[clamp(16px,0.99vw,19px)] leading-[100%] tracking-normal hover:bg-[#1a4050] transition-colors"
          style={{
            width: '150px',
...
ComplaintCard3.propTypes = {
  complaint_id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  case_type: PropTypes.string,
  judge_name: PropTypes.string,
  case_file_number: PropTypes.string,
  status: PropTypes.string,
  submission_date: PropTypes.oneOfType([PropTypes.string, PropTypes.instanceOf(Date)]),
};

ComplaintCard3.defaultProps = {
  complaint_id: "",
  case_type: "",
  judge_name: "",
  case_file_number: "",
