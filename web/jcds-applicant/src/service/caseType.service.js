...
class CaseType {
  async createCaseType(typeData) {
    return apiService
      ...
      .catch((error) => {
        throw new Error(
          error.response?.data.error || "Failed to create case type"
        );
      });
  }
  async getAllCaseTypes() {
    return apiService
      ...
      .catch((error) => {
        throw new Error(
          error.response?.data.error || "Failed to fetch case type"
        );
      });
  }
  async deleteCaseType(typeId) {
    return apiService
      ...
      .catch((error) => {
        throw new Error(
          error.response.data.error || "Failed to delete case type"
        );
      });
  }
  async updateCaseType(typeId, typeData) {
    return apiService
      ...
      .catch((error) => {
        throw new Error(
          error.response.data.error || "Failed to update case type"
        );
      });
  }
