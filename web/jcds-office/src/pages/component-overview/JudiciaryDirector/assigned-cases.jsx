...
const FILTERS = [
  { label: "All", value: "all" },
  { label: "Pending", value: "pending_director_approval" },
  { label: "Case Opened", value: "Opened" },
  { label: "Returned", value: "Returned by Director" },
];
...
export default function AssignedCases() {
  ...
  const fetchAssignedCases = async (case_status = "all") => {
    setLoading(true);
    try {
      const res = await judiciaryDirectorService.getAllAssignedCases({ case_status });
      const data = res.data.map((item, index) => {
        return {
          ...
          evidences_count: item.evidences?.length || 0,
        };
      });
      setRows(data);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchAssignedCases(activeFilter.value);
  }, [tabValue]);

  const handleDetail = (item) => {
    console.log('Navigating to case detail with dispId:', item.disp_Id);
  ...
  const columns = useMemo(() => [
    ...
    { id: "court_office", label: "Court Office", sortable: true },
    {
      ...
      label: "Case Status",
      render: (row) => {
        const meta = getStatusMeta(theme, row.case_status);
        return (
            <Box
              sx={{
          ...
          >
            {meta?.label || row.case_status || "Pending"}
          </Box>
        );
      },
    },
