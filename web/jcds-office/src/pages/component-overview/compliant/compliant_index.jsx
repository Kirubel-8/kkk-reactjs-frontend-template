...
const FILTERS = [
  ...
  { label: "Under Investigation", status: "under_investigation" },
...
const CompliantIndex = () => {
  ...
  const getAllCompliantRequest = async () => {
    ...
    try {
      ...
      const data = await compliantRequestService.getAllRequestCompliantRequest(
        status,
        page + 1,
        rowsPerPage
      );
      console.log({ complaints: data.data });
      setCompliantRequest(data.data || []);
  ...
  const tableRows = useMemo(() => compliantRequest.map((item, index) => ({
    ...item,
    id: item.complaint_id || index,
    rowNumber: page * rowsPerPage + index + 1,
    reporterName: item.applicant?.full_name || "Anonymous",
    caseTypeDisplay: item.case_type || "N/A",
    judgeNameDisplay: item.judge_name || "N/A",
    caseFileNumberDisplay: item.case_file_number || "N/A",
