...
const EditTeam = () => {
  ...
  useEffect(() => {
    // Fetch team data and departments on component mount
    const fetchTeamData = async () => {
      setLoading(true);
      try {
        const [teamData, departmentData] = await Promise.all([TeamService.getTeamById(teamId), DepartmentService.getAllDepartments()]);
        setFormValues({
          name: teamData.name,
        ...
        console.error('Failed to fetch team or departments:', error.message);
        setSnackbar({
          open: true,
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.values(formErrors).every((error) => !error)) {
      setLoading(true);
      try {
        await TeamService.updateTeam(teamId, formValues);
        setSnackbar({
          open: true,
